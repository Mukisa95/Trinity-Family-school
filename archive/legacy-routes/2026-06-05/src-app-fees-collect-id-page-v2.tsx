...
export default function PupilFeesCollectionV2() {
  ...
  useEffect(() => {
    if (academicYears.length > 0 && !selectedAcademicYear) {
      // Find the current active academic year using date-based detection
      const currentYear = detectCurrentAcademicYear(academicYears);
      if (currentYear) {
        setSelectedAcademicYear(currentYear);
        // Find the current term using date-based detection
  ...
  // Fetch pupil details - optimized with cache-first (instant if cached)
