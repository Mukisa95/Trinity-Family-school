...
function ClassRequirementsContent() {
  ...
  React.useEffect(() => {
    if (academicYears.length > 0 && !selectedAcademicYear) {
      ...
      if (effectiveTerm?.academicYear) {
        setSelectedAcademicYear(effectiveTerm.academicYear.id);
        if (effectiveTerm.term && !selectedTerm) {
          setSelectedTerm(effectiveTerm.term.id);
        }
      }
    }
  }, [academicYears, selectedAcademicYear, selectedTerm]);

  // 🚀 OPTIMIZED: Get tracking data for ALL pupils in the class at once
  // This fetches all tracking records for the class/term in a single query (not one pupil at a time)
  // Works exactly like Requirements Tracking page - fetches all data at once
  const trackingQuery = useRequirementTrackingByClass(
    classId || '',
  ...
  const refetch = trackingQuery.refetch;
  ...
  if (!classId) {
    return (
      <div className="p-4 sm:p-6 text-center">
        ...
        <SmartBackButton fallbackHref="/classes" className="mt-4">
  ...
  return (
    <>
      ...
      {/* Requirement Reception Modal - Only render when open to prevent ref issues */}
      {isReceptionModalOpen && selectedPupil && (
