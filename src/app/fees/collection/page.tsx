...
import { PupilFeesRowSkeleton } from '@/components/fees/PupilFeesRowSkeleton';
...
import { isTermEnded, detectCurrentAcademicYear, detectCurrentTerm } from '@/lib/utils/academic-year-utils';
...
export default function FeesCollectionPage() {
  ...
  // Register print handler to open column selection modal
  useEffect(() => {
  ...
  // Skeleton state for class switching
  const previousClassIdRef = useRef<string | undefined>(filters.selectedClassId);
  ...
  // Set initial year and term — use the same effectiveTerm from useTermStatus() as the
  // working fees-collection and exams pages. effectiveTerm is computed via
  // getEffectiveTermForDataDisplay which uses date-fns parseISO/isWithinInterval
  // (timezone-safe) and correctly handles holiday periods by returning the most
  // recently completed term. This avoids the brittle detectCurrentAcademicYear /
  // detectCurrentTerm chain below which uses raw cache data without the sanitiseYear
  // transform.
  useEffect(() => {
    if (effectiveTerm.academicYear?.id && effectiveTerm.term?.id && (!filters.year || !filters.term)) {
      setFilters(prev => ({
        ...prev,
        year: effectiveTerm.academicYear!.id,
        term: effectiveTerm.term!.id
      }));
    }
  }, [effectiveTerm, filters.year, filters.term]);
  ...
  // Handler for class change that shows skeleton immediately
  const handleClassChange = useCallback((classId: string) => {
    // Show skeleton immediately when class changes
    flushSync(() => {
      setShowSkeleton(true);
    });

    // Update refs
    previousClassIdRef.current = filters.selectedClassId;

    // Use startTransition to make the actual class change non-blocking
    startTransition(() => {
      setFilters(prev => ({ ...prev, selectedClassId: classId }));
    });
  ...
  useEffect(() => {
    // Only hide skeleton when:
    // 1. Not loading or fetching
    // 2. We have pupils data
    // 3. Skeleton is currently showing
    const shouldHideSkeleton =
      !isLoadingPupils &&
      !isFetchingPupils &&
      pupils.length > 0 &&
      !isProcessing &&
  ...
  }, [isLoadingPupils, isFetchingPupils, pupils.length, isProcessing, showSkeleton]);
    ...
    return [...filteredPupils].sort((a, b) => {
      ...
      switch (sortConfig.key) {
        ...
        case 'section':
          ...
          return direction * aSection.localeCompare(bSection);
        case 'totalFees':
          if (filters.feeItem) {
  ...
  }, [filteredPupils, sortConfig, pupilFeesInfo, classes, filters.feeItem]);

  // Derive dynamic fee items from the currently visible pupils' actual assigned fees
  // This replaces the static availableFeeStructures list in the filter dropdown
  const dynamicFeeItems = useMemo(() => {
    ...
    // Collect all unique fee items with pupil count
    const feeMap = new Map<string, { id: string; name: string; count: number; isAssignmentFee?: boolean }>();
    for (const pupil of pupilsToScan) {
      const feesInfo = pupilFeesInfo[pupil.id];
      if (!feesInfo?.applicableFees) continue;
      for (const fee of feesInfo.applicableFees) {
        if (!feeMap.has(fee.feeStructureId)) {
          // Look up extra metadata from availableFeeStructures if available
          const meta = availableFeeStructures.find(fs => fs.id === fee.feeStructureId);
          feeMap.set(fee.feeStructureId, {
            id: fee.feeStructureId,
            name: fee.name,
            count: 1,
            isAssignmentFee: meta?.isAssignmentFee
          });
        } else {
          feeMap.get(fee.feeStructureId)!.count++;
        }
      }
    }

    // Return sorted: assignment fees first, then alphabetically by name
    return Array.from(feeMap.values()).sort((a, b) => {
      if (a.isAssignmentFee && !b.isAssignmentFee) return -1;
      if (!a.isAssignmentFee && b.isAssignmentFee) return 1;
      return a.name.localeCompare(b.name);
    });
  }, [pupils, searchQuery, filters.section, filters.class, filters.status, pupilFeesInfo, availableFeeStructures]);
  ...
  const handleGeneratePDF = async () => {
    try {
      ...
      // Calculate filtered totals based on current filters (especially feeItem)
      const calculateFilteredTotals = () => {
        let filteredTotalFees = 0;
        let filteredTotalPaid = 0;
        let filteredBalance = 0;

        sortedPupils.forEach((pupil: any) => {
          const feesInfo = pupilFeesInfo[pupil.id];
          if (!feesInfo) return;

          if (filters.feeItem) {
            // When a specific fee item is selected, use only that fee's data
            const selectedFee = feesInfo.applicableFees?.find(
              (fee: any) => fee.feeStructureId === filters.feeItem
            );
            if (selectedFee) {
              filteredTotalFees += selectedFee.amount || 0;
              filteredTotalPaid += selectedFee.paid || 0;
              filteredBalance += selectedFee.balance || 0;
            }
          } else {
            // When no specific fee item is selected, use total fees
            filteredTotalFees += feesInfo.totalFees || 0;
            filteredTotalPaid += feesInfo.totalPaid || 0;
            filteredBalance += feesInfo.balance || 0;
          }
        });
      ...
      const createPDFComponent = (props: any) => {
        const { pupils, pupilFeesInfo, classes, filters, sortConfig, columnSelection, totals, availableFeeStructures } = props;
        ...
        const getFilterDescription = () => {
          ...
          if (filters.feeItem) {
            const feeItemName = availableFeeStructures?.find((fs: any) => fs.id === filters.feeItem)?.name || 'Unknown Fee';
            filterParts.push(`Fee Item: ${feeItemName}`);
          }
            ...
            >
              ...
              <View style={styles.table}>
                ...
                <View style={styles.tableRow} fixed>
                  ...
                  {columnSelection.balance && (
                    <View style={[styles.tableColHeader, { width: columnWidths.balance }]}>
                      <Text style={styles.tableCellHeader}>
                        {filters.feeItem ? 'Fee Balance' : 'Balance'}
                      </Text>
                    </View>
                  )}
                ...
                {pupils.map((pupil: any, index: number) => {
                  ...
                  // Get filtered fee data based on feeItem filter
                  let displayFees = feesInfo.totalFees || 0;
                  let displayPaid = feesInfo.totalPaid || 0;
                  let displayBalance = feesInfo.balance || 0;
                  ...
                  if (filters.feeItem && feesInfo.applicableFees) {
                    ...
                    if (selectedFee) {
                      displayFees = selectedFee.amount || 0;
                      displayPaid = selectedFee.paid || 0;
                      displayBalance = selectedFee.balance || 0;
                    } else {
                      // Pupil doesn't have this fee item, show zeros
                      displayFees = 0;
                      displayPaid = 0;
                      displayBalance = 0;
                    }
                    ...
                    <View key={pupil.id} style={styles.tableRow} wrap={false} minPresenceAhead={40}>
                      ...
                      {columnSelection.balance && (
                        <View style={[styles.tableCol, { width: columnWidths.balance }]}>
                          <Text style={[styles.tableCell, styles.currencyText, {
                            color: displayBalance > 0 ? '#e53e3e' : '#38a169'
                          }]}>
      ...
      toast({
        title: "PDF Generated",
        description: "Fees collection report is ready for viewing.",
      });
    } catch (error) {
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      {dynamicFeeItems.length === 0 && !isProcessing && (
                        <option disabled value="__none">No fees found for current pupils</option>
                      )}

                      {/* Assignment Fees */}
                      {dynamicFeeItems.filter(f => f.isAssignmentFee).length > 0 && (
                        <optgroup label={`Assignment Fees (${dynamicFeeItems.filter(f => f.isAssignmentFee).length})`}>
                          {dynamicFeeItems.filter(f => f.isAssignmentFee).map(fee => (
                            <option key={fee.id} value={fee.id}>
                              {fee.name} — {fee.count} pupil{fee.count !== 1 ? 's' : ''}
                            </option>
                          ))}
                        </optgroup>
                      )}
                      ...
                      {/* Regular Fees */}
                      {dynamicFeeItems.filter(f => !f.isAssignmentFee).length > 0 && (
                        <optgroup label={`Regular Fees (${dynamicFeeItems.filter(f => !f.isAssignmentFee).length})`}>
                          {dynamicFeeItems.filter(f => !f.isAssignmentFee).map(fee => (
                            <option key={fee.id} value={fee.id}>
                              {fee.name} — {fee.count} pupil{fee.count !== 1 ? 's' : ''}
                            </option>
