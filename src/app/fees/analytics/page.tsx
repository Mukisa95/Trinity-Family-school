...
export default function CollectionAnalyticsPage() {
  ...
  // Get the globally effective term via two-pass cross-year scan (no isActive bias)
  const globalEffective = useMemo(() =>
    getEffectiveTermForDataDisplay(allYears),
    [allYears]
  );

  const defaultYear = globalEffective.academicYear ?? undefined;
  const defaultTermId = globalEffective.term?.id ?? '';
  ...
  //   • If user picked a year manually, use that year's current/most-recent term
  //   • Otherwise use the globally effective term (cross-year two-pass result)
  const effectiveTermId = useMemo(() => {
    if (manualTermId) return manualTermId;
    if (manualYearId) return getCurrentTerm(activeYear as any)?.id || defaultTermId;
    return defaultTermId;
  }, [manualTermId, manualYearId, activeYear, defaultTermId]);
