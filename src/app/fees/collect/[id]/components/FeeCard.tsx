...
export function FeeCard({ fee, pupil, onPayment, onRevertPayment, selectedTerm, selectedAcademicYear, isPaymentDataLoading = false }: FeeCardProps) {
  ...
  const handleCollectionSubmit = async (collectedItems: string[], isFullCollection: boolean, collectionSizes: Record<string, string>) => {
    ...
    try {
      ...
      const allCollectedItems = [...new Set([...previouslyCollected, ...collectedItems])];
      ...
      await updateUniformTracking.mutateAsync({
        ...
        data: {
          ...
          collectionStatus: isFullCollection ? 'collected' : 'partial',
      ...
      <div className="min-w-0">
          ...
          {UniformFeesIntegrationService.isUniformFee(fee) && (
            <div className="mt-2 space-y-2">
              <div className="p-2.5 rounded-md border border-blue-100 bg-blue-50">
                ...
                <div className="text-xs text-blue-700 space-y-1.5">
                  ...
                  {/* Color-coded Item List - Always visible */}
                  <div>
                    <div className="text-xs font-medium text-blue-800 mb-1">Items:</div>
                    <div className="flex flex-wrap gap-1">
                      {(() => {
                        // Get uniform IDs and names from the fee details
                        ...
                        // If we have uniform names, create a map of name to ID
                        if (uniformNames.length > 0) {
                          return uniformNames.map((name: string, index: number) => {
                            // Get uniform ID - try by index first, then by name matching
                            let uniformId = uniformIdArray[index];
                            if (!uniformId && allUniforms.length > 0) {
                              const matchedUniform = allUniforms.find(u => u.name === name);
                              uniformId = matchedUniform?.id;
                            }

                            // Check if this item is collected (only if we have tracking record data)
                            const isCollected = uniformId && previouslyCollectedItems.includes(uniformId);
                            ...
                            return (
                              <div
                                key={index}
                                className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[11px] ${isCollected
                        ...
                        return <div className="text-gray-500 text-[11px]">No items listed</div>;
                      })()}
                    </div>
                  </div>
