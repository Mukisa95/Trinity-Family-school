...
export default function LoginPage() {
  ...
  const galleryPhotos = React.useMemo(() => 
    photos?.filter(p => p.usage.includes('gallery')) || [], 
    [photos]
  );
  const allActivePhotos = React.useMemo(() => 
    photos?.filter(p => p.isActive) || [], 
    [photos]
  );
  const facilityPhotos = React.useMemo(() => 
    photos?.filter(p => p.category === 'facilities' || p.category === 'school_building') || [], 
    [photos]
  );
  const classroomPhotos = React.useMemo(() => 
    photos?.filter(p => p.category === 'classroom') || [], 
    [photos]
  );
  const staffPhotos = React.useMemo(() => 
    photos?.filter(p => p.category === 'staff') || [], 
    [photos]
  );
  const playgroundPhotos = React.useMemo(() => 
    photos?.filter(p => p.category === 'playground') || [], 
    [photos]
  );
  const generalPhotos = React.useMemo(() => 
    photos?.filter(p => p.category === 'other' && p.usage.includes('general')) || [], 
    [photos]
  );
  ...
  // Page loading effect - show content immediately when settings load
  useEffect(() => {
    // Remove artificial delay - page ready when settings are loaded
    if (!settingsLoading) {
  ...
  useEffect(() => {
    if (!isActivityPlaying || allActivePhotos.length <= 1) return;

    const timer = setInterval(() => {
      setCurrentActivitySlide((prev) => (prev + 1) % allActivePhotos.length);
    }, activitySlideSpeed);

    return () => clearInterval(timer);
  }, [isActivityPlaying, activitySlideSpeed, allActivePhotos]);

  // Keyboard navigation for photo slideshow
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (allActivePhotos.length <= 1) return;

      switch (e.key) {
        case 'ArrowLeft':
          e.preventDefault();
          setCurrentActivitySlide((prev) => (prev - 1 + allActivePhotos.length) % allActivePhotos.length);
          break;
        case 'ArrowRight':
          e.preventDefault();
          setCurrentActivitySlide((prev) => (prev + 1) % allActivePhotos.length);
  ...
  }, [allActivePhotos, isActivityPlaying]);
  ...
  // Loading screen component - optimized for performance
  if (pageLoading || settingsLoading) {
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-blue-600 via-purple-600 to-blue-800 flex items-center justify-center z-50">
        ...
        >
          <div className="h-16 w-16 border-4 border-white border-t-transparent rounded-full mx-auto mb-6 animate-spin" />
  ...
  return (
    <div className="relative">
      {/* Removed transition overlay - instant navigation for better UX */}
      ...
      <header className="relative z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border-b shadow-sm">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <LoginDialog open={showLoginDialog} onOpenChange={setShowLoginDialog}>
            ...
            <LoginDialogContent>
              <LoginDialogHeader>
                {/* School Logo/Icon - simplified for performance */}
