...
export default function StaffForm() {
  ...
  const getRoleOptions = (departments: string[] | undefined | null) => {
    // Get all roles for all selected departments
    const allRoles: { value: string; label: string }[] = [];
    
    // Ensure departments is a valid array
  ...
  useEffect(() => {
    if (staffMember && id) {
      ...
      const formattedStaffData: StaffFormData = {
        ...
        department: ensureArray(staffMember.department),
        role: ensureArray(staffMember.role),
  ...
  const generateEmployeeIDFromForm = useCallback(() => {
    try {
      if (!staffData.firstName || !staffData.lastName || !staffData.dateOfBirth || !staffData.department || staffData.department.length === 0) {
              ...
              <div className="space-y-2">
                ...
                {generatedEmployeeID && (
                  <div className="text-xs text-muted-foreground bg-muted/30 px-3 py-2 rounded-lg border border-border/30">
                    <span className="font-medium">Generated from:</span> {staffData.firstName} {staffData.lastName} • {staffData.dateOfBirth} • {staffData.department && Array.isArray(staffData.department) ? staffData.department.join(', ') : ''}
                  </div>
                )}
              </div>
