...
import { PLESnapshotRecaptureModal } from "@/components/ple/PLESnapshotRecaptureModal";
...
export default function PLEResultsPage() {
  ...
  const [isRecaptureModalOpen, setIsRecaptureModalOpen] = React.useState(false);
  const [recordToRecapture, setRecordToRecapture] = React.useState<PLERecord | null>(null);
  ...
  // Recapture handler
  const handleRecapture = (record: PLERecord) => {
    setRecordToRecapture(record);
    setIsRecaptureModalOpen(true);
  };

  const handleRecaptureComplete = () => {
    // Invalidate queries to refresh data
    if (recordToRecapture) {
      queryClient.invalidateQueries({ queryKey: pleResultsKeys.record(recordToRecapture.id) });
      queryClient.invalidateQueries({ queryKey: pleResultsKeys.results(recordToRecapture.id) });
    }
    queryClient.invalidateQueries({ queryKey: pleResultsKeys.records() });

    toast({
      title: "Data Refreshed",
      description: "PLE records have been updated with recaptured snapshots.",
    });
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 animate-in fade-in duration-500">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
                    ...
                    <TableCell className="text-center">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => handleRecapture(record)}>
        ...
        {/* Recapture Snapshots Modal */}
        <PLESnapshotRecaptureModal
          isOpen={isRecaptureModalOpen}
          onClose={() => {
            setIsRecaptureModalOpen(false);
            setRecordToRecapture(null);
          }}
          pleRecord={recordToRecapture}
          onRecaptureComplete={handleRecaptureComplete}
        />
      </div>
    </div>
  );
