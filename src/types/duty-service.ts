...
export interface LeadershipTerm {
  ...
  termName: string;
  startDate: string;
  endDateType: 'mark' | 'open';
  endDate?: string;
  isActive: boolean;
  academicYearId?: string; // Optional linkage if needed
  createdAt: string;
  createdBy: string;
  updatedAt?: string;
}
...
export interface PostAssignment {
  ...
  postId: string;
  pupilId: string;
  leadershipTermId: string; // Made required
  postNameSnapshot?: string; // To preserve historical records if post is deleted
  isActive: boolean;
...
export type CreateLeadershipTermData = Omit<LeadershipTerm, 'id' | 'createdAt' | 'createdBy' | 'updatedAt'>;
export type UpdateLeadershipTermData = Partial<Omit<LeadershipTerm, 'id' | 'createdAt' | 'createdBy'>>;
