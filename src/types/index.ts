...
export type PromotionBatchType = 'Promotion' | 'Demotion' | 'Transfer' | 'Graduation';
...
export interface PromotionBatch {
  ...
  fromClassId: string;
  fromClassName: string;
  toClassId?: string; // Optional for graduations
  toClassName?: string; // Optional for graduations
  pupilIds: string[]; // IDs of pupils promoted in this batch
  pupilCount: number; // Denormalized count for quick display
  processedBy?: string; // User who processed the batch
  academicYearId?: string;
  graduationYear?: number; // For graduation batches
  notes?: string;
  createdAt: string;
}

export type PupilStatus = 'Active' | 'Suspended' | 'Inactive' | 'Graduated' | 'Transferred' | 'Pending' | '';
...
export interface AssignmentStatusHistory {
  date: string; // ISO date string
  action: 'enabled' | 'disabled' | 'time_adjusted' | 'term_pushed' | 'term_fetched';
  ...
  previousTimeSettings?: {
    ...
    excludedTermIds?: string[]; // For explicit exclusions from indefinite assignments
  };
}
...
export interface PupilAssignedFee {
  ...
  excludedTermIds?: string[];   // Any specific terms excluded if termApplicability is 'all_terms'
...
// Fees Holiday Types
export type FeesHolidayDiscountType = 'full' | 'half' | 'quarter' | 'percentage';
export type FeesHolidayCategory = 'required' | 'non-required';

export interface FeesHoliday {
  id: string;
  pupilId: string;
  categories: FeesHolidayCategory[]; // Array of 'required' and/or 'non-required' - allows selecting one or both
  discountType: FeesHolidayDiscountType; // 'full', 'half', 'quarter', or 'percentage'
  discountValue?: number; // Required when discountType is 'percentage' (0-100)
  reason?: string; // Reason for the fees holiday
  isActive: boolean; // Whether the holiday is currently active
  createdAt: string;
  createdBy?: string; // User who created the holiday
  updatedAt?: string;
  updatedBy?: string; // User who last updated the holiday
  disabledAt?: string; // When the holiday was disabled
  disabledBy?: string; // User who disabled the holiday
}
...
export interface Pupil {
  ...
  assignedStaffId?: string; // ID of staff member assigned to this pupil (for Mofus feature)
  ...
  // Performance status for remarks/reports (Legacy flat status)
  performanceStatus?: 'good' | 'fair' | 'weak' | 'young' | 'irregular';

  // Term-based overall performance statuses for nursery assessment reports
  // Key is termId, value is the overall performance status for that term
  termPerformanceStatuses?: Record<string, 'good' | 'fair' | 'weak' | 'young' | 'irregular'>;

  // Subject-based statuses for nursery assessment reports (legacy - kept for backward compatibility)
  subjectStatuses?: Record<SubjectCommentType, SubjectStatus>;

  // Term-based subject statuses for nursery assessment reports
  // Key is termId, value is subject statuses for that term
  termSubjectStatuses?: Record<string, Record<SubjectCommentType, SubjectStatus>>;
...
export type SubjectAssignment = {
  subjectId: string;
  teacherIds: string[]; // Changed from teacherId to support multiple teachers
...
export interface SchoolSettings {
  ...
  pending?: {
    enabled?: boolean;
    affectedComponents?: {
      classes?: boolean; // If true, pending pupils will be EXCLUDED from classes component
      exams?: boolean; // If true, pending pupils will be EXCLUDED from exams component
      attendance?: boolean; // If true, pending pupils will be EXCLUDED from attendance component
      pupilsList?: boolean; // If true, pending pupils will be EXCLUDED from pupils list component
      feesCollection?: boolean; // If true, pending pupils will be EXCLUDED from fees collection component
    };
    timeOfEffect?: 'end_of_term' | 'beginning_of_next_term';
    activationDate?: string; // ISO date string when pending was activated
...
export type SubjectCommentType =
  | 'mathematical_concepts'
  | 'reading'
  | 'writing_concepts'
  | 'god_and_his_creation'
  | 'life_skills'
  | 'vocabulary'
  | 'story_telling'
  | 'general_knowledge'
...
export type SubjectStatus = 'very_good' | 'good' | 'fair' | 'weak';

export interface CommentTemplate {
  id: string;
  status?: 'good' | 'fair' | 'weak' | 'young' | 'irregular'; // Made optional for subject-based comments
  type: 'class_teacher' | 'head_teacher' | 'subject';
  comment: string;
  isActive: boolean;
  createdAt: string;
  updatedAt: string;
  subject?: SubjectCommentType; // For subject-based comments
  subjectStatus?: SubjectStatus; // For subject-based comments
  classId?: string; // Optional class-specific comments

  // Term applicability for subject-based comments
  // Array of term identifiers: 'all', 'term_1', 'term_2', 'term_3'
  // If empty or undefined, applies to all terms (for backward compatibility)
...
export type AttendanceStatus = 'Present' | 'Absent' | 'Late' | 'Excused' | 'Delayed' | '';
...
export interface AttendanceRecord {
  ...
  classCode?: string; // Class code for stable identification
  pupilId: string;
...
export interface ExcludedDay {
  ...
  date?: string;
  description?: string;
  type: 'specific_date' | 'recurring_day_of_week' | 'recurring_monthly' | 'recurring_annual';
  dayOfWeek?: 0 | 1 | 2 | 3 | 4 | 5 | 6; // 0 is Sunday
  dayOfMonth?: number; // 1-31
  monthOfYear?: number; // 1-12 (1 is January)

  // Historical tracking fields
  applicableYearId?: string | 'all'; // If specific string, only applies to that year
  startYearId?: string; // Rule only applies from this academic year onwards
  endYearId?: string; // Rule stops applying after this academic year (soft delete)
  skippedYearIds?: string[]; // Academic years where this recurring rule was explicitly deleted
...
export interface Notification {
  ...
  // Delete tracking (soft delete for "delete for me")
  deletedBy?: Record<string, boolean>; // Map of user IDs who have deleted this notification
...
export interface Dormitory {
  ...
  classIds: string[]; // Classes eligible for this dormitory
  bedCapacity: number;
  patronStaffIds: string[]; // Staff in charge
  inChargePupilIds: string[]; // Pupil leaders
  assignedPupilIds?: string[]; // Pupils currently assigned
  createdAt: string;
  updatedAt?: string;
}

// School Houses (for inter-house system)
export interface House {
  id: string;
  name: string;
  motto?: string;
  themeColor: string; // HEX color e.g., #FF0000
  createdAt: string;
  updatedAt?: string;
}
...
export interface EventExamIntegration {
  ...
  examName: string;
  examType?: string; // Optional exam type ID/Name
  examNature?: string; // Optional 'Set based' or 'Subject based'
  maxMarks: number;
  passingMarks: number;
...
export interface TimetableProfile {
  id: string;
  name: string; // E.g. "Upper Primary Timetable"
  classIds: string[]; // Which classes this table applies to
  academicYearId: string;
  termId: string;
  ...
  timeBlocks?: {
    id: string;
    type: 'lessons' | 'break';
    count?: number; // For lessons
    duration?: number; // For breaks
    label?: string; // e.g. "BREAK", "LUNCH"
  }[];

  // Old fixed fields (optional for backward compatibility if any exist)
  lessonsBeforeBreak?: number;
  breakDuration?: number;
  lessonsAfterBreak?: number;
  lunchDuration?: number;
  lessonsAfterLunch?: number;

  activeDays: number[]; // e.g., [1, 2, 3, 4, 5, 6] (Mon-Sat)
  createdAt: string;
  createdBy?: string;
  updatedAt?: string;
}

export type PeriodType = 'lesson' | 'break' | 'lunch' | 'assembly' | 'custom';

export interface GeneratedPeriod {
  id: string; // auto-generated
  dayOfWeek: number; // 1 = Monday, etc.
  periodNumber: number; // Sequential 1, 2, 3...
  startTime: string; // "06:30"
  endTime: string; // "07:30"
  type: PeriodType;
  customLabel?: string; // e.g., "Health Parade"
}
...
export interface TimetableEntry {
  ...
  entryType?: 'subject' | 'activity';
  subjectId: string;
  teacherId: string;
  coTeacherId?: string; // Optional
  optionalSubjectId?: string; // For split classes (e.g. French/Computer)
  optionalTeacherId?: string;
  coOptionalTeacherId?: string;
  /** For co-curricular activities — the display name (e.g. "Assembly", "Prayers") */
  activityName?: string;
  /** For group activities — other classIds sharing this exact activity in the same period */
