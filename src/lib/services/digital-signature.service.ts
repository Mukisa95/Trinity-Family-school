...
export class DigitalSignatureService {
  ...
  static formatActionDescription(recordType: RecordType, action: string, metadata?: Record<string, any>): string {
    const actionMappings: Record<RecordType, (action: string, meta?: any) => string> = {
      ...
      // Duty and Service Mappings
      leadership_term_creation: (action, meta) => `Leadership term created: ${meta?.termName || 'term'}`,
      leadership_term_update: (action, meta) => `Leadership term updated: ${meta?.termName || 'term'}`,
      leadership_term_termination: (action, meta) => `Leadership term terminated`,
      prefectoral_post_creation: (action, meta) => `Prefectoral post created: ${meta?.postName || 'post'}`,
      prefectoral_post_update: (action, meta) => `Prefectoral post updated: ${meta?.postName || 'post'}`,
      prefectoral_post_deletion: (action, meta) => `Prefectoral post deleted`,
      post_assignment_creation: (action, meta) => `Pupil assigned to post`,
      post_assignment_deletion: (action, meta) => `Post assignment deleted`,
      duty_rota_creation: (action, meta) => `Duty rota created: ${meta?.dutyName || 'rota'}`,
      duty_rota_update: (action, meta) => `Duty rota updated: ${meta?.dutyName || 'rota'}`,
      duty_rota_deletion: (action, meta) => `Duty rota deleted`,
      duty_assignment_creation: (action, meta) => `Member assigned to duty`,
      duty_assignment_update: (action, meta) => `Duty assignment updated`,
      duty_assignment_deletion: (action, meta) => `Duty assignment deleted`,
      bulk_duty_assignment: (action, meta) => `Bulk duty assignment created`,
      poll_creation: (action, meta) => `Poll created: ${meta?.title || 'poll'}`,
      poll_vote: (action, meta) => `Vote cast in poll`,
      performance_ranking_creation: (action, meta) => `Performance ranking created: ${meta?.rankingPeriod || 'period'}`
    };
