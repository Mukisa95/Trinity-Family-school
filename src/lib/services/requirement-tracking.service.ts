...
export class RequirementTrackingService {
  ...
  static async getTrackingRecordsByPupilAndTerm(pupilId: string, academicYearId: string, termId: string): Promise<RequirementTracking[]> {
    try {
      console.log('🔍 Service: getTrackingRecordsByPupilAndTerm called with:', { pupilId, academicYearId, termId });
      const q = query(
        collection(db, COLLECTION_NAME),
        where('pupilId', '==', pupilId),
        where('academicYearId', '==', academicYearId),
      ...
      console.log('🔍 Service: Executing Firestore query...');
      const querySnapshot = await getDocs(q);
      console.log('🔍 Service: Query returned', querySnapshot.docs.length, 'documents');
      ...
      console.log('🔍 Service: Returning', sortedRecords.length, 'sorted records');
      return sortedRecords;
    } catch (error) {
      console.error('❌ Service: Error fetching requirement tracking records by pupil and term:', error);
      throw error;
    }
  }
  ...
  static async getEnhancedTrackingRecordsByPupilAndTerm(
    ...
    try {
      ...
      let records = await this.getTrackingRecordsByPupilAndTerm(pupil.id, academicYear.id, termId);
      
      // If no records found with strict query, try fetching all records for pupil and filter client-side
      // This handles cases where termId might not match exactly
      if (records.length === 0) {
        console.log('⚠️ No records found with strict query, trying fallback: fetch all pupil records');
        const allPupilRecords = await this.getTrackingRecordsByPupil(pupil.id);
        records = allPupilRecords.filter(record => 
          record.academicYearId === academicYear.id && 
          record.termId === termId
        );
        console.log('📊 Fallback query results:', {
          allPupilRecordsCount: allPupilRecords.length,
          filteredRecordsCount: records.length
        });
      }
      
      console.log('📊 Final tracking records:', {
