  ...
  static async updateFeeStructure(id: string, data: UpdateFeeStructureData): Promise<FeeStructure> {
    try {
      ...
      await HistoryLogService.log({
        ...
        entity: 'fee_structure',
        recordId: id,
        label: updatedFeeStructure.name,
        changedFields: Object.keys(cleanedData).filter(key => key !== 'updatedAt'),
        meta: {
          amount: updatedFeeStructure.amount,
          termId: updatedFeeStructure.termId || '',
          yearId: updatedFeeStructure.academicYearId || '',
        },
      });
  ...
  static async deleteFeeStructure(id: string): Promise<void> {
    try {
      ...
      await HistoryLogService.log({
        ...
        entity: 'fee_structure',
        recordId: id,
        label: feeStructure?.name || id,
        meta: {
          amount: feeStructure?.amount || 0,
          termId: feeStructure?.termId || '',
          yearId: feeStructure?.academicYearId || '',
        },
      });
