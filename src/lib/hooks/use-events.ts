...
export function useEvents(filters?: EventFilters) {
  ...
  const liteEvents = liteRead<Event[]>(LITE_KEYS.events);

  return useQuery({
    queryKey: ['events', filters],
    queryFn: async () => {
      // 1. Check in-memory cache first (populated by preloader)
      const cachedData = queryClient.getQueryData<Event[]>(['events', undefined]);
      
      let sourceEvents: Event[] | undefined = undefined;

      if (cachedData && cachedData.length > 0) {
        sourceEvents = cachedData;
      } else if (liteEvents && liteEvents.length > 0) {
        sourceEvents = liteEvents;
      }

      if (sourceEvents) {
        // If we have no filters, return all
        if (!filters || Object.keys(filters).length === 0) {
          return sourceEvents;
        }

        // Apply filters locally instead of hitting Firestore
        ...
        if (filters.academicYearIds?.length) {
          filteredEvents = filteredEvents.filter(e => filters.academicYearIds!.includes(e.academicYearId));
        }

        if (filters.isExamEvent !== undefined) {
          filteredEvents = filteredEvents.filter(e => e.isExamEvent === filters.isExamEvent);
        }
      ...
      // 2. Cold start fallback (Firestore read)
      try {
    ...
    initialData: () => {
      const globalEvents = queryClient.getQueryData<Event[]>(['events', undefined]) || liteEvents;
      ...
      if (globalEvents && globalEvents.length > 0) {
        ...
        // Apply filters locally for initialData
    ...
    initialDataUpdatedAt: liteEvents ? Date.now() : undefined,
  });
}
