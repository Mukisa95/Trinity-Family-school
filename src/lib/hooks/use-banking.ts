...
export function useAccountsWithPupils() {
  // ✅ NO real-time listener — data loads when banking page opens and is cached.
  // Refreshes on each visit to the banking page (refetchOnMount: true).
  return useQuery({
    queryKey: bankingKeys.accountsWithPupils(),
    queryFn: BankingService.getAllAccountsWithPupils,
    staleTime: 5 * 60 * 1000,   // 5 minutes — stays fresh within a session
    gcTime: 30 * 60 * 1000,     // 30 minutes — kept in memory when not viewing
    refetchOnMount: true,        // Refresh when banking page is opened
    refetchOnWindowFocus: false, // Don't re-read just from switching tabs
...
export function useAccountByPupilId(pupilId: string) {
  // ✅ NO real-time listener — fetches on demand when a pupil account page opens.
  return useQuery({
    ...
    refetchOnMount: true,        // Refresh when pupil account page is opened
