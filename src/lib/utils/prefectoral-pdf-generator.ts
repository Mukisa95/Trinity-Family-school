...
const generateListViewPDF = async (
  ...
  const columnWidths = {
    0: availableWidth * 0.10,  // Rank (10%)
    1: availableWidth * 0.35,  // Post Name (35%)
    2: availableWidth * 0.35,  // Current Holder (35%)
    3: availableWidth * 0.10,  // Class (10%)
    4: availableWidth * 0.10   // Allowance (10%)
  };
  ...
  autoTable(doc, {
    head: [
      ['Rank', 'Post Name', 'Current Holder', 'Class', 'Allowance']
    ],
    body: tableData,
    startY: headerSpace,
    styles: {
...
const drawAppStyleCard = (
  ...
  // 🚀 VALIDATION: Ensure valid dimensions before drawing
  if (!cardX || !cardY || !cardWidth || !cardHeight || 
      cardWidth <= 0 || cardHeight <= 0 ||
      isNaN(cardX) || isNaN(cardY) || isNaN(cardWidth) || isNaN(cardHeight)) {
    console.warn('Invalid card dimensions:', { cardX, cardY, cardWidth, cardHeight });
    return;
  }
