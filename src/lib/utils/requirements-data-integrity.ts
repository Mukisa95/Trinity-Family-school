...
export function isTermValidForPupil(
  ...
  // Term is valid if the pupil was registered on or before the term's end date
  // This allows pupils who register mid-term to still receive requirements for that term
...
export function isPupilValidForTerm(
  ...
  // Pupil should appear if they were registered on or before the term's end date
  // This allows pupils who register mid-term to still appear in requirements for that term
  return registrationDate <= termEndDate;
}
  ...
  for (const year of sortedYears) {
    ...
    if (year.id === currentAcademicYear.id) {
      ...
      for (let i = 0; i < currentTermIndex; i++) {
        ...
        if (term) {
          // Check if term is valid for pupil (registration on or before term end date)
          if (pupilRegistrationDate && !isTermValidForPupil(term, pupilRegistrationDate)) {
            console.log(`⏭️ Skipping term ${term.name}: pupil registered after term ended`);
            continue;
          }
...
export function filterApplicableRequirements(
  ...
  if (!isTermValidForPupil(currentTerm, pupil.registrationDate)) {
    console.log(`❌ Term ${currentTerm.name} is not valid for pupil (pupil registered after term ended. Registration: ${pupil.registrationDate}, Term end: ${currentTerm.endDate})`);
    return [];
  }
...
export function validateRequirementTrackingCreation(
  ...
  if (!isTermValidForPupil(currentTerm, pupil.registrationDate)) {
    return { 
      isValid: false, 
      reason: `Pupil registered after term ${currentTerm.name} ended (registration: ${pupil.registrationDate}, term end: ${currentTerm.endDate})` 
    };
  }
