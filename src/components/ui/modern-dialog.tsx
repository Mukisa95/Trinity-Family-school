...
const ModernDialogContent: React.FC<ModernDialogContentProps> = ({
  ...
  React.useEffect(() => {
    ...
    if (open) {
      ...
      // Don't block body scrolling - allow page to scroll behind dialog
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[99999] flex items-start justify-center overflow-y-auto py-4 sm:py-8">
      {/* Backdrop */}
      <div
      ...
      >
        ...
        {noPadding ? (
          <>
            {!hasDialogTitle && (
              <VisuallyHidden><h2 id="dialog-title">Dialog</h2></VisuallyHidden>
            )}
            {!hasDialogDescription && (
              <VisuallyHidden><p id="dialog-description">Dialog content</p></VisuallyHidden>
            )}
            {children}
          </>
        ) : (
