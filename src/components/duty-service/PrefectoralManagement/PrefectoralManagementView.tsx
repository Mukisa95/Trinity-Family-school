...
export function PrefectoralManagementView({
  ...
  // Use the hook to get the term object if one is selected
  const { data: terms } = useLeadershipTerms();
  const terminateTermMutation = useTerminateLeadershipTerm();

  const selectedTerm = selectedTermId ? terms?.find(t => t.id === selectedTermId) : undefined;

  const handleTerminateTerm = () => {
    if (selectedTermId) {
      terminateTermMutation.mutate(selectedTermId);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          {selectedTerm && selectedTerm.isActive && selectedTerm.endDateType === 'open' && (
            <Button onClick={handleTerminateTerm} disabled={terminateTermMutation.isPending} variant="destructive" size="sm" className="h-8">
              {terminateTermMutation.isPending ? 'Terminating...' : 'Terminate Open Term'}
            </Button>
          )}
        </div>
      ...
      {showAssignModal && selectedPostForAssignment && (
        <AssignPupilToPostModal
          ...
          onOpenChange={(open) => {
            ...
            if (!open) setSelectedPostForAssignment(null);
          }}
