...
export function DutyTimelineView({ rotaId }: DutyTimelineViewProps) {
  ...
  const { data: posts = [] } = usePrefectoralPosts(); // Modified this line to remove argument
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {timeline.periods.map((period: DutyPeriod) => (
                  <TableRow key={period.periodNumber}>
                    ...
                    <TableCell>
                      <div className="text-sm text-muted-foreground max-w-xs">
                        {period.assignments.find((a: DutyAssignment) => a.service)?.service || 'No service description'}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="space-y-1">
                        {period.assignments.length === 0 ? (
                          ...
                          (() => {
                            const staffAssignments = period.assignments.filter((a: DutyAssignment) => a.memberType === 'staff');
                            const prefectAssignments = period.assignments.filter((a: DutyAssignment) => a.memberType === 'prefects');
                            const pupilAssignments = period.assignments.filter((a: DutyAssignment) => a.memberType === 'pupils');
                                          ...
                                          <>
                                            ...
                                            {prefectAssignments.length > 0 && (
                                              <div>
                                                ...
                                                <div className="space-y-2">
                                                  {prefectAssignments.map((assignment: DutyAssignment) => (
