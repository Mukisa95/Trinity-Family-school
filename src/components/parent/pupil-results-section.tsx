...
export function PupilResultsSection({ pupilId, pupilName, className = '' }: PupilResultsSectionProps) {
  ...
  // Helper function to determine the most relevant term to display
  const getMostRelevantTerm = (yearId: string) => {
    const yearTerms = groupedResults[yearId];
    if (!yearTerms) return 'all';

    // Get available terms for this year, sorted by term number
    const termNames = Object.keys(yearTerms).sort((a, b) => {
      // Extract term numbers (e.g., "Term 1" -> 1, "Term 2" -> 2)
      const aNum = parseInt(a.match(/\d+/)?.[0] || '0');
      const bNum = parseInt(b.match(/\d+/)?.[0] || '0');
      return aNum - bNum;
    });

    if (termNames.length === 0) return 'all';

    // If we have an active academic year with terms, find the most recently completed term
    if (activeAcademicYear && activeAcademicYear.id === yearId && activeAcademicYear.terms) {
      const now = new Date();
      let mostRecentCompletedTerm: string | null = null;

      // Check each term to find the most recent one that has ended
      for (const term of activeAcademicYear.terms) {
        ...
        if (termEndDate <= now && termNames.includes(term.name)) {
          mostRecentCompletedTerm = term.name;
        }
      }

      // If we found a recently completed term with results, use it
      if (mostRecentCompletedTerm) {
        console.log(`📊 Results: Defaulting to most recently completed term: ${mostRecentCompletedTerm}`);
        return mostRecentCompletedTerm;
      }
    }

    // Fallback: Return the latest term (highest term number) with results
    const latestTerm = termNames[termNames.length - 1];
    console.log(`📊 Results: Defaulting to latest available term: ${latestTerm}`);
    return latestTerm;
  };
  ...
  useEffect(() => {
    if (results.length > 0 && !selectedYear && !selectedTerm) {
      // Try to set current academic year and most relevant term
      if (activeAcademicYear && availableYears.includes(activeAcademicYear.id)) {
        setSelectedYear(activeAcademicYear.id);
        const relevantTerm = getMostRelevantTerm(activeAcademicYear.id);
        setSelectedTerm(relevantTerm);
      } else {
        // Fallback to most recent year and most relevant term
        const mostRecentYear = availableYears.find(year => year !== 'all');
        if (mostRecentYear) {
          setSelectedYear(mostRecentYear);
          const relevantTerm = getMostRelevantTerm(mostRecentYear);
          setSelectedTerm(relevantTerm);
        }
      }
    }
