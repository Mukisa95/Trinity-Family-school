...
export function RequirementReceptionModal({
  ...
  // const { data: activeAcademicYear } = useActiveAcademicYear(); // Removed in favor of dynamic detection
  const { data: academicYears = [] } = useAcademicYears();

  // 🚀 DYNAMIC YEAR & TERM DETECTION
  ...
  // Initialize selected academic year and term with active values
  useEffect(() => {
  ...
  }, [activeAcademicYear, currentTerm, selectedAcademicYearId, selectedTermId]);
  ...
  // Reset state when modal closes and refetch when modal opens
  useEffect(() => {
    if (!isOpen) {
      setItems([]);
      setErrors({});
      prevEligibleReqIdsRef.current = '';
      prevTrackingRecordIdsRef.current = '';
      prevTermIdRef.current = '';
      // Reset to active academic year and current term when modal closes
    ...
    // When modal opens, refetch tracking records to ensure fresh data
    if (isOpen && selectedTermId && selectedAcademicYear) {
      // Refetch to get the latest data from the database
      refetch().catch(console.error);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, selectedTermId, selectedAcademicYear?.id]);

  // Get eligible requirements for this pupil with enhanced data integrity
  // Use selected academic year and term
  const { data: eligibleRequirements = [], isLoading: requirementsLoading } = useEnhancedRequirementsByFilter(
    pupil,
    selectedTermId,
    selectedAcademicYear || null,
    academicYears // Pass all academic years for proper filtering
  );

  // Get existing tracking records with enhanced data integrity
  const { data: trackingRecords = [], refetch, isLoading: trackingLoading } = useEnhancedRequirementTrackingByPupilAndTerm(
    pupil,
    selectedTermId,
    selectedAcademicYear || null
  );
  ...
  const prevEligibleReqIdsRef = React.useRef<string>('');
  const prevTrackingRecordIdsRef = React.useRef<string>('');
  const prevTermIdRef = React.useRef<string>('');

  // Initialize items when modal opens or data changes
  useEffect(() => {
    if (!isOpen || !selectedTermId) {
      if (items.length > 0) {
        setItems([]);
        setErrors({});
      }
    ...
    if (requirementsLoading || trackingLoading) {
      return;
    }

    // Debug logging
    console.log('📦 Requirement Reception Modal - Data loaded:', {
      eligibleRequirementsCount: eligibleRequirements.length,
      ...
      trackingRecords: trackingRecords.map(r => ({
        ...
        itemQuantityReceived: r.itemQuantityReceived,
        itemQuantityProvided: r.itemQuantityProvided
      }))
    });

    // Create stable string keys for comparison
    const eligibleReqIds = eligibleRequirements.map(req => req.id).sort().join(',');
    const trackingRecordIds = trackingRecords.map(record => record.id).sort().join(',');
    const currentKey = `${selectedTermId}-${eligibleReqIds}-${trackingRecordIds}`;
    const prevKey = `${prevTermIdRef.current}-${prevEligibleReqIdsRef.current}-${prevTrackingRecordIdsRef.current}`;

    // Only update if something actually changed
    if (currentKey === prevKey && items.length > 0) {
      return; // No changes, skip update
    }

    // Update refs
    prevEligibleReqIdsRef.current = eligibleReqIds;
    prevTrackingRecordIdsRef.current = trackingRecordIds;
    ...
    if (eligibleRequirements.length > 0) {
      const receptionItems: RequirementReceptionItem[] = eligibleRequirements.map(requirement => {
        const existingRecord = trackingRecords.find(record => {
          const reqIds = Array.isArray(record.requirementId)
            ...
            : [record.requirementId];
          const matches = reqIds.includes(requirement.id);
          if (matches) {
            console.log('✅ Matched requirement:', {
              requirementId: requirement.id,
              requirementName: requirement.name,
              recordId: record.id,
              itemQuantityReceived: record.itemQuantityReceived,
              itemQuantityProvided: record.itemQuantityProvided
            });
          }
          return matches;
        });
      ...
      // No eligible requirements for selected term/year
      setItems([]);
      setErrors({});
    }
    // Depend on requirements and tracking records so it updates when data changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, selectedTermId, eligibleRequirements, trackingRecords, requirementsLoading, trackingLoading]);
  ...
  const handleSubmit = async () => {
    if (!validateSubmission() || !selectedAcademicYear || !selectedTermId) return;
    ...
    try {
      ...
      // Close modal - cache invalidation will handle data refresh
      // The modal will refetch fresh data when reopened
      onClose();
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Select
                    ...
                    disabled={!selectedAcademicYearId || selectedYearTerms.length === 0}
                  >
                    <SelectTrigger className="h-8 text-xs w-full">
